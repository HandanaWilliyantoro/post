...
function normalizeAccount(row, overrides = {}) {
  ...
  return serializeAccounts([{
    ...
    assignmentStatus:
      overrides.assignmentStatus ?? resolveAssignmentStatus(campaignSlug),
    campaignSlug,
    ...campaignFields,
  }])[0];
}
...
export async function listIdleAccounts() {
  const collection = await ensureAccountsCollection();
  return collection
    .find({ campaignSlug: "", status: ACTIVE_ACCOUNT_STATUS })
    .project({ _id: 0 })
    .sort({ username: 1 })
    .toArray()
    .then(serializeAccounts);
...
export async function getCampaignAccounts(campaignSlug) {
  ...
  return collection
    .find({ campaignSlug: slug, status: ACTIVE_ACCOUNT_STATUS })
    .project({ _id: 0 })
    .sort({ username: 1 })
    .toArray()
    .then(serializeAccounts);
