...
export async function findCampaignBySlug(slug) {
  ...
  const slugifiedSlug = slugifyCampaign(normalizedSlug);

  if (!normalizedSlug && !slugifiedSlug) {
    return null;
  }

  const collection = await ensureCampaignsCollection();
  const slugCandidates = [...new Set([normalizedSlug, slugifiedSlug].filter(Boolean))];
...
export async function updateCampaign(slug, input = {}) {
  const normalizedSlug = normalizeText(slug || input.slug);
  const label = normalizeText(input.label);
  ...
  await collection.updateOne({ slug: normalizedSlug }, { $set: updates });

  return normalizeCampaignDocument({
    ...existingCampaign,
    ...updates,
  });
}
...
export async function deleteCampaign(slug) {
  ...
  const collection = await ensureCampaignsCollection();
  const db = await getDb();
  const activeBulkPublishRun = await db.collection("progress").findOne(
    {
      jobId: "bulk-publish",
      campaignSlug: normalizedSlug,
      status: { $in: ["queued", "running", "cancelling"] },
    },
    { projection: { _id: 0, runId: 1 } }
  );

  if (activeBulkPublishRun) {
    throw new Error(
      "Campaign cannot be deleted while a bulk publish run is active"
    );
  }
