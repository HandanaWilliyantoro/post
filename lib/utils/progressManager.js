...
export async function listProgressRuns(options = {}) {
  ...
  // Recent runs must always be scoped to one campaign when listing history
  if (!campaignSlug && !String(options?.runId || "").trim()) {
    return [];
  }
  ...
  return runs
    ...
    .filter((run) => {
      ...
      return String(run?.campaignSlug || "").trim() === campaignSlug;
    });
}
