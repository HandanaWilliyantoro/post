...
const BULK_PUBLISH_RETRY_BASE_DELAY_MS = Math.max(
  1000,
  Number(process.env.BULK_PUBLISH_RETRY_BASE_DELAY_MS || 5 * 1000) || 5 * 1000
);
const BULK_PUBLISH_RETRY_MAX_DELAY_MS = Math.max(
  BULK_PUBLISH_RETRY_BASE_DELAY_MS,
  Number(process.env.BULK_PUBLISH_RETRY_MAX_DELAY_MS || 5 * 60 * 1000) ||
    5 * 60 * 1000
);
const BULK_PUBLISH_RETRY_WAIT_STEP_MS = 1000;
// 0 = keep retrying retryable failures until success or cancel
const BULK_PUBLISH_MAX_RETRY_ATTEMPTS = (() => {
  const raw = process.env.BULK_PUBLISH_MAX_RETRY_ATTEMPTS;
...
function buildWaveScheduleJobs(videos, accounts, captions = [], options = {}) {
  ...
  const publishSlots = buildPublishSlotsFromBase({
    ...
    basePublishAt: resolvePublishAtForIndex(
      basePublishAt,
      options.campaignSlug || options.campaign
    ),
    count: waveCount,
  });
  const matchType =
...
function canRetryBulkPublishAttempt(retryAttemptCount) {
  if (BULK_PUBLISH_MAX_RETRY_ATTEMPTS === 0) {
    return true;
  }

  return (
    Math.max(0, Number(retryAttemptCount || 0) || 0) <=
    BULK_PUBLISH_MAX_RETRY_ATTEMPTS
  );
}

function getRetryDelayMs(retryAttemptCount, error = null) {
  const clientRetryAfterMs = Number(error?.retryAfterMs);

  if (Number.isFinite(clientRetryAfterMs) && clientRetryAfterMs >= 0) {
    return Math.min(
      BULK_PUBLISH_RETRY_MAX_DELAY_MS,
      Math.max(1000, clientRetryAfterMs)
    );
  }
  ...
  if (retryAfterHeader != null && retryAfterHeader !== "") {
    ...
    if (Number.isFinite(asSeconds) && asSeconds >= 0) {
      return Math.min(
        BULK_PUBLISH_RETRY_MAX_DELAY_MS,
        Math.max(1000, Math.ceil(asSeconds * 1000))
      );
    }
  ...
  const base =
    status === 429
      ? Math.max(BULK_PUBLISH_RETRY_BASE_DELAY_MS, 15 * 1000)
      : BULK_PUBLISH_RETRY_BASE_DELAY_MS;
  const jitter = Math.floor(Math.random() * 500);

  return Math.min(
    BULK_PUBLISH_RETRY_MAX_DELAY_MS,
    base * 2 ** exponent + jitter
  );
}
...
function resolvePublishAtForIndex(basePublishAt, campaignContext = null) {
  ...
  const normalized = baseEastern.toUTC().toISO({ suppressMilliseconds: false });

  if (!campaignContext) {
    return normalized;
  }

  return snapPublishAtToScheduleSlot(normalized, campaignContext);
}
...
async function resolveAutomaticBasePublishAt(campaignSlug) {
  ...
  return resolveDefaultCampaignPublishAt({
    campaignSlug,
    lastPublishAt: latestPublishAt,
  });
}
...
export async function bulkPublishPipeline({
  ...
  try {
    ...
    if (isRetryRun) {
      ...
      resolvedPublishAt = resolvePublishAtForIndex(
        resolvedPublishAt,
        campaignSlug
      );
    ...
    async function processJob(job) {
      const publishAtForJob =
        job.publishAt ||
        resolvePublishAtForIndex(resolvedPublishAt, campaignSlug);
      ...
      try {
        ...
        if (blockingDuplicate || (isRetryRun && isAlreadyScheduledError(error))) {
          ...
          const retryable =
            ...
            canRetryBulkPublishAttempt(nextRetryAttemptCount);
          const nextRetryAtMs = retryable
            ? Date.now() + getRetryDelayMs(nextRetryAttemptCount, error)
            : null;
