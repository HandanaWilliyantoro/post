...
import { createSocialPost } from "@/lib/postforme/posts";
...
export async function persistPostRecord(post, options = {}) {
  ...
  const status =
    ...
    "scheduled";
  const localOnly = normalizeBoolean(
    options.localOnly ?? post?.localOnly,
  ...
  const document = {
    ...
    origin: post?.origin || options.origin || existingRecord?.origin || "local",
    ...
    failure: null,
    syncedAt: now,
    publishCallback: localOnly
      ? buildDisabledPublishCallback()
      : buildPendingPublishCallback(),
  };
...
export async function createPost(payload) {
  try {
    ...
    const { campaignSlug, signal, ...postPayload } = payload || {};
    ...
    const duplicateKey = String(postPayload?.duplicateKey || "").trim();
    const {
    ...
    const providerPayload = {
      ...
      ...(duplicateKey && !requestBase.external_id
        ...
        : {}),
      content: String(requestBase.content || "").trim(),
      publish_at: requestBase.publish_at || null,
      targets: normalizeTargets(postPayload.targets),
      media: normalizeMedia(postPayload.media),
      source_file_path: postPayload.source_file_path || null,
    };
    const created = await createSocialPost(providerPayload, { signal });

    if (!created?.post?.id) {
      throw new Error("Invalid PostForMe post response");
    }

    return persistPostRecord(created.post, {
      ...postPayload,
      media: created.media,
