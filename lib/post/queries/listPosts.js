...
const ACTIVE_MANUAL_POST_STATUSES = ["queued", "running"];
...
function buildPublishHourExpr(options = {}) {
  const hour = normalizePublishHourFilter(options.publishHour);

  if (hour === null) {
    return null;
  }

  // Match publish_at hour in Eastern time (7 / 15 / 23)
  return {
    $eq: [
      {
        $hour: {
...
export async function syncPastScheduledPostStatuses(options = {}) {
...
export async function cleanupCampaignPosts(campaignSlug) {
  ...
  const collection = await ensurePostsCollection();
  const activeManualPostQuery = {
    localOnly: true,
    origin: "manual",
    "manualJob.type": "manual-post",
    "manualJob.status": { $in: ACTIVE_MANUAL_POST_STATUSES },
  };
  const [activeManualPostsPreserved, result] = await Promise.all([
    collection.countDocuments({
      campaignSlug: normalizedCampaignSlug,
      ...activeManualPostQuery,
    }),
    collection.deleteMany({
      campaignSlug: normalizedCampaignSlug,
      $nor: [activeManualPostQuery],
    }),
  ]);
  ...
  return {
    postsDeleted: result.deletedCount || 0,
    activeManualPostsPreserved,
  };
}
...
export async function listPostsPage(options = {}) {
  ...
  const publishHourExpr = buildPublishHourExpr(options);
  const statusQuery = buildStatusQuery(options);
  const regex = queryText ? new RegExp(escapeRegex(queryText), "i") : null;

  await syncPastScheduledPostStatuses({ collection, campaignSlug });
  ...
  const query = {
    ...
    ...(regex
      ...
      : {}),
    ...(publishHourExpr ? { $expr: publishHourExpr } : {}),
  };
