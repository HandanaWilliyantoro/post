...
export async function getPostMetrics(campaignSlug) {
  const posts = await listAllPosts({ campaignSlug });
  const queuedStatuses = new Set(["queued", "scheduled"]);
  const queuedPosts = posts.filter((post) =>
    queuedStatuses.has(String(post?.status || "").trim().toLowerCase())
  );
