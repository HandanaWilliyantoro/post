...
function normalizeMediaType(value, fallback = "video") {
  ...
  return ["image", "video"].includes(normalized) ? normalized : fallback;
}
...
function buildRetryMedia(post) {
        ...
        .map((item) =>
          item?.url
            ? {
                ...
                type: normalizeMediaType(item?.type, "video"),
              }
            : null
        )
  ...
  return mediaUrl
    ? [{ url: mediaUrl, type: normalizeMediaType(post?.manualJob?.mediaType, "video") }]
    : [];
}
...
async function processManualPost(post) {
  ...
  const mediaType = normalizeMediaType(post?.manualJob?.mediaType, "video");
  ...
  if (!sourceFilePath && !retryMedia.length) {
    throw new Error("Queued manual post is missing its uploaded media");
  }

  try {
    const resolvedMedia = retryMedia.length
      ? retryMedia
      : [{ url: sourceFilePath, type: mediaType }];
    ...
    const createdPost = await createPost({
      ...
      duplicateKey,
      source_file_path: sourceFilePath || post?.source_file_path || null,
    ...
    await updateManualJobState(
      duplicateKey ? { duplicateKey } : { id: createdPost.id },
      {
        ...
        "manualJob.filePath": sourceFilePath || null,
        "manualJob.lastError": "",
      }
    );
  } catch (error) {
...
export async function queueManualPost(options = {}) {
  const queuedPost = await persistQueuedPostRecord({
    ...
    media: Array.isArray(options.media) && options.media.length
      ? options.media
      : [{ type: normalizeMediaType(options.mediaType, "video") }],
    ...
    manualJob: {
      ...
      mediaType: normalizeMediaType(options.mediaType, "video"),
      lastError: "",
...
export async function retryFailedPost(postId) {
  ...
  return queueManualPost({
    ...
    targets,
    media: retryMedia,
    mediaType: normalizeMediaType(existingPost?.manualJob?.mediaType, "video"),
    fileName,
